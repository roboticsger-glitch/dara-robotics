...
export default async function Home() {
  ...
  return <>
    ...
    <section className="section dark-section"><div className="container"><div className="section-heading inverse"><div><span className="eyebrow">INDUSTRIES</span><h2>Automation across industries.</h2></div><p>From hospitality to industrial sites, every deployment is designed around ROI and operations.</p></div><div className="industry-grid">{industries.map(([Icon,title,desc]) => { const I = Icon as typeof Hotel; return <div className="industry-card" key={String(title)}><I/><h3>{String(title)}</h3><p>{String(desc)}</p></div>})}</div></div></section>
